...
def _user(username: str, *, is_admin: bool = False) -> AuthenticatedUser:
    return AuthenticatedUser(username=username, is_admin=is_admin)
...
class _FakeQueryOrchestrator:
    """Mirrors QueryOrchestrator's public shape without any SQL routing -
    equivalent to what QueryOrchestrator itself does when
    ``RagRuntimeConfig.sql_enabled`` is False (the default), which is the
    only path ChatController's own tests need to exercise. SQL-routing
    behavior is covered separately in
    tests/query_orchestration/test_query_orchestrator.py."""

    def __init__(self, rag_service: _FakeRAGService) -> None:
...
def test_chat_returns_rag_response_unchanged() -> None:
    ...
    result = controller.chat(_user("alice"), ChatRequest(question="what is the policy?"))
...
def test_chat_forwards_retrieval_mode_override_to_rag_service() -> None:
    ...
    controller.chat(
        _user("alice"), ChatRequest(question="what is the policy?", retrieval_mode="hybrid")
    )
...
def test_chat_persists_a_history_entry_for_the_caller() -> None:
    ...
    controller.chat(_user("alice"), ChatRequest(question="what is the policy?"))
...
def test_chat_history_write_failure_does_not_crash_the_request() -> None:
    ...
    result = controller.chat(_user("alice"), ChatRequest(question="q"))
...
def test_get_history_returns_only_the_caller_own_entries_most_recent_first() -> None:
    ...
    controller.chat(_user("alice"), ChatRequest(question="alice q1"))
    controller.chat(_user("bob"), ChatRequest(question="bob q1"))
    controller.chat(_user("alice"), ChatRequest(question="alice q2"))
...
def test_get_history_respects_limit_and_offset() -> None:
    controller, *_ = _controller()
    for i in range(5):
        controller.chat(_user("alice"), ChatRequest(question=f"q{i}"))
...
def test_chat_without_conversation_id_creates_a_new_conversation() -> None:
    ...
    result = controller.chat(_user("alice"), ChatRequest(question="what is the refund policy?"))
...
def test_chat_title_is_truncated_at_a_word_boundary() -> None:
    ...
    controller.chat(_user("alice"), ChatRequest(question=long_question))
...
def test_chat_with_conversation_id_reuses_it_instead_of_creating_a_new_one() -> None:
    ...
    first = controller.chat(_user("alice"), ChatRequest(question="first question"))

    second = controller.chat(
        _user("alice"),
        ChatRequest(question="second question", conversation_id=first.conversation_id),
    )
...
def test_chat_with_conversation_id_owned_by_another_user_raises_not_found() -> None:
    ...
    someone_elses = controller.chat(_user("bob"), ChatRequest(question="bob's question"))
...
def test_chat_with_nonexistent_conversation_id_raises_not_found() -> None:
    ...
    with pytest.raises(ConversationNotFoundError):
        controller.chat(_user("alice"), ChatRequest(question="q", conversation_id=999))
...
def test_list_conversations_filters_by_search() -> None:
    controller, *_ = _controller()
    controller.chat(_user("alice"), ChatRequest(question="refund policy question"))
    controller.chat(_user("alice"), ChatRequest(question="branch hours question"))
